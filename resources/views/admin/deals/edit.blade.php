    ...
    <div class="flex min-h-screen bg-[#fdfaf8]">
        ...
        <main class="flex-1 p-8">
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-3xl font-bold text-[#5C4033]">Edit Deal</h1>
                ...
                <form action="{{ route('admin.deals.update', $deal) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="category" id="category" value="{{ old('category', $deal->category) }}"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#5C4033] focus:border-transparent">
                            @error('category')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="text" name="discount" id="discount" value="{{ old('discount', $deal->discount) }}" placeholder="e.g., 50% OFF"
                        ...
                        <div>
                            ...
                            <input type="text" name="distance" id="distance" value="{{ old('distance', $deal->distance) }}" placeholder="e.g., 1 Mile away"
                                   required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#5C4033] focus:border-transparent">
                            @error('distance')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="description" id="description" rows="3"
                                      class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#5C4033] focus:border-transparent">{{ old('description', $deal->description) }}</textarea>
                            @error('description')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            @if($deal->image)
                                <div class="mb-2">
                                    <img src="{{ asset('storage/' . $deal->image) }}" alt="{{ $deal->title }}" class="h-20 w-20 object-cover rounded">
